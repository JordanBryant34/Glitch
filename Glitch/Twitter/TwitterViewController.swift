...
class TwitterViewController: UIViewController {
    ...
    let segmentControl: CustomSegmentedControl = {
        ...
        sc.insertSegment(withTitle: "Feed", at: 0, animated: false)
        sc.insertSegment(withTitle: "Profile", at: 1, animated: false)
     ...
     let segmentBar: UIView = {
        let view = UIView()
        view.backgroundColor = .twitterLightBlue()
        view.layer.masksToBounds = false
    ...
    lazy var feedController: TwitterFeedController = {
        let controller = TwitterFeedController()
        return controller
    }()
    
    lazy var profileController: TwitterProfileViewController = {
        let controller = TwitterProfileViewController()
        controller.screenName = "TwitterSignedInUser"
        return controller
    }()
    ...
    override func viewDidLoad() {
        ...
        add(asChildViewController: feedController)
    ...
    @objc private func handleSegmentChange() {
        if segmentControl.selectedSegmentIndex == 0 {
            remove(asChildViewController: profileController)
            add(asChildViewController: feedController)
        } else {
            remove(asChildViewController: feedController)

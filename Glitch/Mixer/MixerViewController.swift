...
//  Social.ly
//
//  Created by Jordan Bryant on 2/9/20.
//  Copyright © 2020 Jordan Bryant. All rights reserved.
//

import UIKit

class MixerViewControler: UIViewController {
    
    let segmentControl: CustomSegmentedControl = {
        let sc = CustomSegmentedControl(items: [])
        sc.insertSegment(withTitle: "Browse", at: 0, animated: false)
    ...
    let segmentBar: UIView = {
        let view = UIView()
        view.backgroundColor = .mixerLightBlue()
        view.layer.masksToBounds = false
    ...
    lazy var browseController: MixerBrowseController = {
        let viewController = MixerBrowseController()
        return viewController
    }()
    
    lazy var followingController: MixerFollowingController = {
        let viewController = MixerFollowingController()
        return viewController
    }()
    ...
    override func viewDidLoad() {
        ...
        add(asChildViewController: browseController)
    ...
    @objc private func handleSegmentChange() {
        if segmentControl.selectedSegmentIndex == 0 {
            ...
            remove(asChildViewController: browseController)
